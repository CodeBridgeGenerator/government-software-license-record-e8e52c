...
import sheet2FakerFactory from "./sheet2FakerFactory";
...
const Sheet2FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sheet2FakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sheet2").create(elem));
...
export default connect(mapState, mapDispatch)(Sheet2FakerDialogComponent);
