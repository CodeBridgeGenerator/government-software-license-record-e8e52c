...
const Sheet2DataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.ethnicity}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.population}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.percentage}</p>
...
<Column field="population" header="Population" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
<Column field="percentage" header="Percentage" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Sheet2 Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
...
export default Sheet2DataTable;